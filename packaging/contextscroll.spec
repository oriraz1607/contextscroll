...
# The daemon was already stripped and verified in the release bundle. Preserve
# its exact bytes so the RPM's executable matches the attested archive.
...
Source0:        contextscroll-v%{version}-linux-%{_target_cpu}.tar.gz
...
%prep
%setup -q -n contextscroll-v%{version}-linux-%{_target_cpu}
